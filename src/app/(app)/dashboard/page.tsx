...
import { AlertTriangle, TrendingUp, ChefHat } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [produtosRes, cardapioRes] = await Promise.all([
            ...
            return (
              <div key={dia.short} style={{ background: '#FAFAF8', borderRadius: '10px', padding: '10px', border: '1px solid #E5E3DC' }}>
                <div style={{
                  ...
                  borderRadius: '6px', padding: '3px 0', marginBottom: '8px',
                ...
                <div style={{ marginBottom: '8px' }}>
                  ...
                  {almocoLinhas.slice(0, 3).map((linha, i) => (
                    <div key={i} style={{ fontSize: '11px', color: '#2C2C2A', lineHeight: 1.4, marginBottom: '1px' }}>{linha}</div>
                  ))}
                  {almocoLinhas.length > 3 && <div style={{ fontSize: '10px', color: '#888780' }}>+{almocoLinhas.length - 3} itens</div>}
                </div>
                ...
                <div>
                  ...
                  {jantarLinhas.slice(0, 2).map((linha, i) => (
                    <div key={i} style={{ fontSize: '11px', color: '#5F5E5A', lineHeight: 1.4, marginBottom: '1px' }}>{linha}</div>
                  ))}
                  {jantarLinhas.length > 2 && <div style={{ fontSize: '10px', color: '#888780' }}>+{jantarLinhas.length - 2} itens</div>}
                </div>
              </div>
            )
          })}
